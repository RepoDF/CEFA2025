...
def calculate_kstest_new(long_short_signal_returns):
    """
    Calculate the KS-Test against the signal returns with a long or short signal.
    ...
    for i in long_short_signal_returns.index.unique(): 
        mean = long_short_signal_returns.loc[i,:].mean(axis = 0)
        sd = long_short_signal_returns.loc[i,:].std(axis = 0)
        norm_arg = (mean, sd)
        ks = np.append(ks,kstest(long_short_signal_returns['signal_return'],'norm',norm_arg)[0])
        p =np.append(p,kstest(long_short_signal_returns['signal_return'],'norm',norm_arg)[1])
        
    ks_values = pd.Series(ks,index = long_short_signal_returns.index.unique())
    p_values = pd.Series(p ,index = long_short_signal_returns.index.unique())
    return ks_values, p_values
